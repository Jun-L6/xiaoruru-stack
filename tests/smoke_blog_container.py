...
def main():
    ...
    docker("run", "-d", "--name", name, "--label", "xiaoruru.qa=" + name,
           ...
           "-e", "SERVER_TOMCAT_THREADS_MAX=16", "-e", "SERVER_TOMCAT_THREADS_MIN_SPARE=2",
    ...
    try:
        ...
        for path in ("/", "/admin/login"):
            ...
            response.read()
            conn.close()
        print("Blog image healthy without CPA; UID 10001; read-only root; 512 MiB memory limit; homepage/login 200.")
    finally:
        info = json.loads(docker("inspect", name))[0]
